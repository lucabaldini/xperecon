#include "TEventDisplay_new_v3.h"
TEventDisplay::TEventDisplay(const TGWindow *p, const TGWindow *main,
			     UInt_t w, UInt_t h):TGTransientFrame(p, main, w, h)
{ 
  
  SmallCRadius = WideCRadius = 0;
  if (!gSystem->AccessPathName( "ClustersData.root", kFileExists )) {
    //Open data file 
    Clusterdatafile = new TFile("ClustersData.root", "r");
    cout << "==>> Opened ClustersData.root for data selection and display" << endl;
    InitializeTree();
    BookHistos(); 
  }
  else {
  ...
  if (!gSystem->AccessPathName( "RawSignals.root", kFileExists )) {
    RawSignalFile = new TFile("RawSignals.root", "r");
  ...
  if (!gSystem->AccessPathName( "infodisplay.dat", kFileExists )) { // reads last settings of the display control panel
    ReadInfoFile();
  }
  else {
...
void TEventDisplay::BookHistos(){
  ...
  TH2F *fCumulativetHisto = new TH2F("CumulativeHitMap", "CumulativeHitMap", 346, -7.5, 7.5, 300, -7.5, 7.5 );
  fCumulativetHisto->GetXaxis()->SetTitle("X position (mm)");
  fCumulativetHisto->GetYaxis()->SetTitle("Y position (mm)");
  obj = fCumulativetHisto;
  histList->Add(obj);
...
void TEventDisplay::ReadPixmap(){
 // Draw the border pixels;
  ...
  if (!gSystem->AccessPathName( "pixmap.dat", kFileExists )) {
    ...
    index = 0;
    lflag = hflag =0;  
    PixmapFile1.open("pixmap.dat", ios::in);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TEventDisplay::ReadInfoFile(){ 
    infodispIn.open("infodisplay.dat",ios::in);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void TEventDisplay::WriteInfoFile(){
  infodispOut.open("infodisplay.dat",ios::out);
...
void TEventDisplay::SelectAnalisedFile(){
  const char *filetypes[] = {"ROOT files",    "ClustersData*.root",			      
			     0,               0 };
  static TString dir(".");  
  using namespace std;
  TGFileInfo fi1;
  ...
  if(Clusterdatafile) Clusterdatafile->Close();
  TString cludata = fi1.fFilename;
  if(cludata=="") return; 
  cout << "Load output file: " << cludata << endl;
  dir = fi1.fIniDir;
  Clusterdatafile = new TFile(cludata, "r");
  InitializeTree();
  BookHistos();
}
...
void TEventDisplay::DrawHistos(){
  ...
  TEllipse *el = new TEllipse(XEllipseCut,YEllipseCut,R1EllipseCut,R2EllipseCut,0.,360.,ThetaEllipseCut);
  ...
  cout << "NumberOfEvts: " << NumberOfEvts << " --- Entries in PH histos after cuts: " << Entries << endl; 
 
  TCanvas *nCanvas = new TCanvas("secondstepMod","Modulation second step");
  ...
  ((TH1F*)histList->At(5))->Draw();
  nCanvas->Print("secondstepMod.png");
  return;
}
